...
class KettleCard extends LitElement {
  ...
  constructor() {
    ...
    this._targetTemp = 0;
    this._arcAngle = 855; // Начальный угол
  }
  ...
  static get styles() {
    return css`
      ...
      .circle-progress {
        ...
        box-sizing: border-box;
        transform: rotate(855deg); /* Начальный угол */
  ...
  willUpdate(changedProperties) {
    if (changedProperties.has('hass') && this.hass && this.config) {
      ...
      if (this._targetTemp !== targetTemp) {
        ...
        this._arcAngle = 855 + ((targetTemp - 40) / 60) * (45 - 855); // От 855° до 45°
      }
    }
  }
  ...
  render() {
    ...
    // Рассчитываем угол дуги (от 855° до 45°)
    const angle = 855 + progress * (45 - 855); // 45° - 855° = -810°
  ...
  updateTemperatureFromEvent(e) {
    ...
    // Корректируем угол (от 855° до 45°)
    if (degree < 45) degree += 360;
    if (degree > 855) degree -= 360;
    
    // Ограничиваем диапазон
    degree = Math.max(45, Math.min(855, degree));
    
    // Преобразуем угол в температуру
    const minTemp = 40;
    const maxTemp = 100;
    const tempRange = maxTemp - minTemp;
    const angleRange = 810; // 855° - 45° = 810°
    const temp = Math.round(minTemp + ((degree - 45) / angleRange) * tempRange);
